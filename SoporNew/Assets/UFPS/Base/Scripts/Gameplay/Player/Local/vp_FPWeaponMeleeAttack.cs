	...
	protected void UpdateAttack()
	{
		...
		vp_Timer.In(SwingDelay, delegate()
		{
			...
			vp_Timer.In(ImpactTime, delegate()
			{
				...
				{
                    var intractiveObject = hit.collider.gameObject.GetComponent<InteractiveObject>();
                    if (intractiveObject != null)
                    {
                        intractiveObject.PlayerInteract(_gameManager.Player._currentWeapon, _gameManager, hit.point);
                        _gameManager.Player.MainHud.InventoryPanel.QuickSlotsPanel.CurrentSlot.ItemModel.ChangeDurability(1);
                        if (_gameManager.Player.MainHud.InventoryPanel.QuickSlotsPanel.CurrentSlot.ItemModel == null)
                        {
                            _gameManager.Player.SetEmptyHand();
                            SoundManager.PlaySFX(WorldConsts.AudioConsts.BreakWeapon);
                        }
                    }
                    else
                    {
                        var animalLink = hit.collider.gameObject.GetComponent<AnimalColliderLink>();
                        if (animalLink != null)
                        {
                            animalLink.SetDamage(_gameManager.Player._currentWeapon.Item.Damage, hit.point);
                            _gameManager.Player.MainHud.InventoryPanel.QuickSlotsPanel.CurrentSlot.ItemModel.ChangeDurability(1);
