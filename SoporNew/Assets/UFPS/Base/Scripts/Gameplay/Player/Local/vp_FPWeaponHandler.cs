...
public class vp_FPWeaponHandler : MonoBehaviour
{
    ...
    public void SetWeaponSmooth(int weaponIndex = 0)
    {
        vp_Timer.In(SetWeaponDuration + 0.1f, delegate()
        {
            if (!m_Player.SetWeapon.TryStart(weaponIndex))
                Debug.LogWarning("Warning (" + this + ") Requested 'StartWeapon' (" + m_Weapons[weaponIndex].name + ") was denied, likely by the inventory. Make sure it's present in the inventory from the beginning.");
        });
    }
