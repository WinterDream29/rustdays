    ...
    public class SpawnAlien : MonoBehaviour
    {
        ...
        public void OnTogglePushed(bool WireOn)
        {
            ...
            if (WireOn)
            {
                ...
                GameObject.Find(GameManager.GetCurrent() + "(Clone)")
                    .GetComponentInChildren<SkinnedMeshRenderer>()
                    .enabled = true;
                GameObject.Find(GameManager.GetCurrent() + "(Clone)").GetComponentInChildren<Renderer>().sharedMaterial
                    = newMat;
                state = true;
            }
            else
            {
                Material newMat =
                    Resources.Load(
                        "Materials/" + GameManager.GetCurrent().Substring(0, 5) +
                        Regex.Match(GameManager.GetCurrent(), @"\d+").Value + "Mat", typeof(Material)) as Material;
